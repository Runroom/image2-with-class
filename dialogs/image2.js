            ...
            children: [
              ...
              },
              {
                ...
                html: lockResetHtml
					...
					//},
            {
              // Add property to have 2 diferent layouts for the images
              // configurable via classes without inline styles
              type: 'hbox',
              id: 'ratio',
              requiredContent: features.align.requiredContent,
              children: [
                {
                  id: 'align',
                  type: 'radio',
                  // harcode copies and classes
                  // TODO paramatrize it
                  items: [
                    [ lang.layoutLandscape, 'img_landscape' ],
                    [ lang.layoutPortrait, 'img_portrait' ] ],
                  label: lang.layout,
                  setup: function( widget ) {
                    // tmpclass is used for changes and open again
                    // in the same editor session. If tmpclass is set
                    // should be used it
                    if(widget.data.tmpclass != undefined) {
                      this.setValue(widget.data.tmpclass);
                    } else {
                      // if no tmpclass is seted and no class in the
                      // loaded html (p.e. new image)
                      if(widget.data.classes == null) {
                        this.setValue('img_landscape');
                      } else {
                        // if there is som class seted
                        if(widget.data.classes.img_portrait != undefined) {
                          this.setValue('img_portrait');
                        } else if (widget.data.classes.img_landscape != undefined) {
                          this.setValue('img_landscape');
                        } else {
                          // no class seted predefine class with the
                          // aspect ratio
                  ...
                  commit: function( widget ) {
                    ...
                    widget.removeClass('img_landscape');
                    widget.removeClass('img_portrait');
                    // set new class
